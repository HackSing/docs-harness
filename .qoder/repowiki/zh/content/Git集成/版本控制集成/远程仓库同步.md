...
## 更新摘要
**所做更改**   
- 新增自动检测拉取文件的git_sync_landed_scope功能说明
- 更新推送策略章节，详细说明增量推送中的自动文件检测机制
- 增强详细组件分析部分，包含git diff --name-status的解析逻辑
- 更新故障排除指南，包含git_sync_landed_scope相关的调试信息
...
**更新** 新增了git_sync_landed_scope自动检测功能，通过git diff --name-status自动识别拉取的文件并纳入变更范围管理。
...
- **新增** git_sync_landed_scope自动检测：通过git diff --name-status自动识别拉取的文件变更。
...
- [scripts/harness.py:4960-4982](file://scripts/harness.py#L4960-L4982)
...
Note over H,G : 新增：git diff --name-status自动检测拉取文件
```
...
- **新增** 自动检测拉取文件：在git_sync重准入流程中，通过git diff --name-status自动比较old_head和new_head的差异，提取变更文件路径并加入git_sync_landed_scope。
...
LFSSub --> |可用| AutoDetect["自动检测拉取文件<br/>git diff --name-status"]
AutoDetect --> Snapshot["生成快照/指纹/范围"]
Snapshot --> End(["结束"])
Block --> End
BlockFF --> End
BlockDirty --> End
...
- [scripts/harness.py:680-794](file://scripts/harness.py#L680-L794)
- [scripts/harness.py:4960-4982](file://scripts/harness.py#L4960-L4982)
- [tests/test_harness.py:5554-5597](file://tests/test_harness.py#L5554-L5597)
...
- **新增** 自动范围扩展：当检测到git_sync漂移时，自动将拉取的文件纳入write_scope，避免重复准入流程。
...
**章节来源**
- [scripts/harness.py:664-678](file://scripts/harness.py#L664-678)
...
- **新增** 自动范围检测优化：git_sync_landed_scope仅在head变化时执行diff操作，避免不必要的计算开销。
...
- **新增** 智能diff触发：仅在git_sync重准入且head发生变化时执行git diff --name-status，避免冗余计算。
...
- **新增** git_sync_landed_scope相关排查：
  - 检查git_sync_landed_scope是否正确包含拉取的文件路径。
  - 验证write_scope是否自动包含了git_sync_landed_scope的内容。
  - 确认git diff --name-status命令执行成功且返回正确的变更路径。
- 调试步骤：
  ...
  - 检查 task-package.json 中的 git_sync_landed_scope 字段内容。
...
**更新** 新增的git_sync_landed_scope自动检测功能显著提升了git_sync操作的自动化程度，通过git diff --name-status自动识别拉取的文件变更，减少了手动配置和重复准入的需求，提高了整体工作效率。
...
- 监控指标建议：
  ...
  - **新增** git_sync_landed_scope计数：track自动检测到的拉取文件数量。
- 配置开关：
  - verification.command_cache_enabled：整体关闭验证命令缓存。
  - verification.auto_attribute_in_scope：关闭 write_scope 内自动归因。
- **新增** git_sync_landed_scope工作流程：
  - 检测old_head与new_head差异
  - 执行git diff --name-status获取变更路径
  - 合并到git_sync_landed_scope数组
  - 自动扩展到write_scope
  - 避免重复准入流程
