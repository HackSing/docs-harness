...
- 测试套件：tests/test_harness.py（覆盖Git fetch/sync/inspect等关键路径）
...
U->>CLI : run --target . --task "... git_sync ..."
CLI->>CLI : 意图分类/变更面判定
...
- 元数据写入（git_metadata_write）：git_fetch仅允许声明的远端refs/objects变化，HEAD、索引与工作区必须保持不变。
- 工作区写入（workspace_write）：git_sync需绑定单一预检OID，自动生成新增/修改/删除/重命名范围，且要求fast-forward与无脏工作区重叠。

```mermaid
flowchart TD
Start(["开始"]) --> Intent["意图分类<br/>query/audit/git_inspect/git_fetch/git_sync"]
Intent --> Profile{"变更面判定"}
Profile --> |read_only| ReadOnly["只读许可<br/>allowed_actions包含git_inspect"]
Profile --> |git_metadata_write| MetaWrite["元数据写入许可<br/>git_fetch受限范围"]
Profile --> |workspace_write| WSWrite["工作区写入许可<br/>git_sync需计划与范围"]
MetaWrite --> Prefetch["预检: 远端对象存在/LFS/Submodule可用"]
...
- 测试用例覆盖fetch/sync/inspect关键路径，确保契约行为稳定。
