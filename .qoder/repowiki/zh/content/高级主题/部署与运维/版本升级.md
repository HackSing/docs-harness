...
## 更新摘要
**变更内容**   
- 版本号从 1.6.6 升级到 1.6.7
- 新增 repowiki 外部知识库支持，实现只消费模式
- 增强可观测性，支持知识卡片截断状态监控
- 完善测试覆盖，包含完整的 repowiki 功能验证
...
**更新** 本次升级从 v1.6.6 到 v1.6.7，主要增强了 repowiki 知识库管理能力，改进了系统可观测性，并提供了全面的测试覆盖。
...
G[".qoder/repowiki/<br/>外部知识库(可选)"] --> A
```
...
- **新增** repowiki 外部知识库支持
  - 当项目存在 `.qoder/repowiki` 目录时，进入只消费模式，不创建或更新本地知识库。
  - 支持知识卡片枚举、范围匹配和上下文解析。
...
participant RepoWiki as ".qoder/repowiki(可选)"
Dev->>CLI : "project upgrade --target <项目>"
CLI->>Proj : "读取当前状态与配置"
CLI->>Rules : "校验规则快照与指纹"
CLI->>Contracts : "校验契约与版本一致性"
CLI->>RepoWiki : "检测外部知识库(可选)"
CLI-->>Dev : "输出差异与影响面(预览)"
...
**更新** v1.6.7 版本中，VERSION 常量已更新为 "1.6.7"，package.json 和 SKILL.md 中的版本也已同步更新。
...
- **新增** repowiki 兼容性检查
  - 如果项目存在 `.qoder/repowiki` 目录，需要确认外部知识库的可用性和完整性。
  - 验证知识卡片的 frontmatter 格式和 scope 配置是否正确。
...
- **新增** repowiki 模式处理
  - 检测到 `.qoder/repowiki` 目录时，自动进入只消费模式，跳过本地知识库初始化。
  - 验证知识卡片枚举和范围匹配功能正常工作。
...
- **新增** repowiki 故障恢复
  - 如果 repowiki 外部知识库不可用，系统会自动降级到本地知识库模式。
  - 验证知识卡片限制和环境变量配置是否正确。
...
- **新增** repowiki 验证
  - 如果存在 `.qoder/repowiki` 目录，验证外部知识库可用性和知识卡片格式。
  - 确认知识卡片限制环境变量配置正确。
...
- **新增** repowiki 相关问题
  - 如果 repowiki 外部知识库不可访问，检查网络连接和权限配置。
  - 验证知识卡片 frontmatter 格式是否符合规范，scope 配置是否正确。
  - 调整 DOCS_HARNESS_REPOWIKI_CARD_LIMIT 环境变量解决卡片数量限制问题。
...
Controller --> RepoWiki[".qoder/repowiki/ 外部知识库(可选)"]
...
- **新增** repowiki 性能优化
  - 知识卡片枚举支持限制数量，避免大知识库的性能问题。
  - 支持环境变量配置卡片限制，适应不同规模的项目需求。
...
- **新增** repowiki 故障排查
  - 检查 `.qoder/repowiki` 目录是否存在且可读。
  - 验证知识卡片 frontmatter 格式和 scope 配置。
  - 调整 DOCS_HARNESS_REPOWIKI_CARD_LIMIT 环境变量解决卡片限制问题。
...
## 结论
Docs Harness 的版本升级以控制器为核心，结合受管规则与契约实现安全、可审计与可回滚的升级流程。通过严格的版本一致性校验、兼容性保证与完整的检查清单，确保升级过程稳定可靠。v1.6.7 版本新增的 repowiki 外部知识库支持进一步增强了系统的灵活性和可扩展性。建议在每次升级前充分准备与验证，遇到问题及时回滚并修复。
...
- **新增** repowiki 相关配置
  - DOCS_HARNESS_REPOWIKI_CARD_LIMIT: 设置知识卡片数量限制
  - 支持的环境变量和配置选项
