...
- [docs/plans/task-admission-efficiency-plan.md](file://docs/plans/task-admission-efficiency-plan.md)
...
- docs/contracts.md：v1.6.5 合同定义，包含 task-package/v2、evidence-receipt/v2、verification-command-receipt/v1、上下文与授权收据等
...
- [scripts/harness.py:2150-2399](file://scripts/harness.py#L2150-L2399)

## 架构总览
下图展示从 run 到 verify 的主流程，以及缓存与幂等的关键节点。
...
participant Cache as "收据与内容缓存"
Host->>Ctrl : run(--target, --task, --facts)
Ctrl->>Cache : active_task_key 幂等检查
alt 命中活动任务
...
## 性能考量
- 缓存命中率提升：通过内容寻址与验证命令收据缓存显著降低重复执行与上下文加载
...
## 结论
通过内容寻址缓存、验证命令收据缓存与上下文正文复用，结合活动任务幂等、状态锁与分段指纹模型，Docs Harness 在保持安全边界的前提下显著降低了重复工作与重新准入成本。配合事件遥测与量化指标，可实现持续的性能优化与问题定位。
