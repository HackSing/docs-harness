...
图表来源 
- [harness.py:1488-1498](file://scripts/harness.py#L1488-L1498)
- [harness.py:8419-8430](file://scripts/harness.py#L8419-L8430)
...
- [harness.py:8419-8430](file://scripts/harness.py#L8419-L8430)
- [harness.py:8373-8402](file://scripts/harness.py#L8373-L8402)
...
章节来源
- [harness.py:8697-8750](file://scripts/harness.py#L8697-L8750)
...
- [harness.py:6313-6346](file://scripts/harness.py#L6313-L6346)
...
章节来源
- [harness.py:8316-8370](file://scripts/harness.py#L8316-L8370)

### 子命令：list/status/prepare/dispatch/progress/verify/retry 统一语法
- 通用参数：--target（必填）、--json（输出 JSON）
- 子命令特有参数：
  - list：无额外参数
  - status：--job-id
  - prepare：--job-id、[--repair]
  - dispatch：--job-id、--job-status（dispatched|running）
  - progress：--job-id、--work-package-id、--work-package-status（in_progress|completed|blocked）、[--reason-code]
  - verify：--job-id、--assessment
  - retry：--job-id
...
## 结论
background 命令组提供了完整的后台任务治理能力，涵盖 Job 生命周期管理、工作包状态机、进度跟踪与验收流程。通过严格的控制面保护、状态转移校验与事件审计，确保后台任务的可控性与可追溯性。建议在生产环境中结合监控与告警，及时发现并处理异常状态。
