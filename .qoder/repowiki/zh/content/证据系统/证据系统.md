...
## 更新摘要
**变更内容**   
- 新增 docs-harness/evidence-declaration/v1 证据声明草案支持
- 增强 workspace_attribution 自动归因收据功能
- 改进 git drift 场景处理机制
- 优化验证命令执行流程和错误处理
...
**更新** 新增了证据声明草案支持、自动工作区归因功能和改进的 git drift 处理机制。
...
**更新** 新增了 workspace_attribution 证据类型用于自动工作区归因。
...
**更新** 增强了自动归因功能，现在支持更智能的工作区写入追踪和 git drift 场景处理。
...
**更新** 改进了 git drift 场景处理，现在能更好地处理远端同步和自动归因。
...
**更新** 新增了 evidence-declaration/v1 支持，允许宿主提交简化的证据声明草案，控制器会自动转换为完整的 v2 收据。
...
**更新** 新增了 workspace_attribution 证据类型的支持，适配器需要能够处理自动归因场景。
...
**更新** 自动归因功能的引入进一步提升了验证效率，减少了人工干预需求。
...
- 常见错误与处置：
  ...
  - 自动归因失败：检查 verification.auto_attribute_in_scope 配置。
- 调试建议：
  - 查看 events.jsonl 中的 verification_attempt 事件；
  - 检查命令收据缓存命中情况；
  - 核对 active rule content fingerprint 与冻结规则一致；
  - 监控 auto_attribution 事件日志。

**更新** 新增了自动归因相关的故障排查指导。

**章节来源**
- [docs/contracts.md:153-164](file://docs/contracts.md#L153-164)
- [docs/contracts.md:190-221](file://docs/contracts.md#L190-L221)
...
**更新** 新增的证据声明草案支持和自动工作区归因功能进一步简化了证据提交流程，改进了 git drift 场景处理能力，使系统更加健壮和易用。
...
- 规则示例与证据类型映射：
  ...
  - 工作区自动归因 → workspace_attribution

**更新** 新增了 workspace_attribution 证据类型的映射。
