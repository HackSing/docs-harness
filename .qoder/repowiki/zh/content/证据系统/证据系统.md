...
## 更新摘要
**所做更改**   
- 新增证据清单与预生成骨架机制，消除首轮补证往返
- 扩展证据声明格式以支持功能确认集成
- 增强知识地图集成功能，支持功能级确认配置
- 完善收据条件说明和模板路径管理
...
**更新** 新增了证据清单机制和预生成骨架功能，通过 `evidence_checklist` 字段在准入响应中提供一次性备齐的证据要求，包含必需证据类型、条件性证据、所需收据及其条件说明，以及预生成的证据骨架文件路径，显著减少首轮补证往返。
...
### 证据清单与预生成骨架机制
**新增功能** 证据系统在任务准入阶段自动生成证据清单和预生成骨架文件，消除首轮补证往返：

- **证据清单结构**：
  - `required`：完成清单要求的必需证据类型数组
  - `conditional`：预声明条件性证据及其激活条件
  - `required_receipts`：每项收据附带条件性文字说明
  - `skeletons`：准入时预生成的证据骨架文件路径数组

- **预生成骨架**：
  - 为每个必需证据类型生成 `templates/evidence-{type}-skeleton.json` 文件
  - 骨架包含完整的 schema_version、type、空 write_set/read_set、conclusion 字段
  - 内置 `_instructions` 指导用户如何正确填写证据内容
  - 准入与验收使用同一助手函数，保证文件一致性
...
- [scripts/harness.py:3010-3030](file://scripts/harness.py#L3010-L3030)

**章节来源**
- [scripts/harness.py:3033-3048](file://scripts/harness.py#L3033-L3048)
- [scripts/harness.py:3010-3030](file://scripts/harness.py#L3010-L3030)
- [tests/test_harness.py:7110-7139](file://tests/test_harness.py#L7110-L7139)
...
**图表来源** 
- [docs/contracts.md:190-221](file://docs/contracts.md#L190-221)
...
### 功能确认支持与知识地图集成
**新增功能** 系统现在支持通过知识地图集成功能确认机制：

- **功能确认配置**：
  - 知识地图中的功能可以声明 `functional_confirmation` 配置
  - 支持 tier、mode、assertions、testing_ref、required、skip_reason 等字段
  - 系统自动检查匹配功能的确认要求

- **智能过滤机制**：
  - 自动跳过测试文件和文档文件的变更
  - 对 `.md`、`.test.ts`、`.spec.ts` 等测试文件后缀自动忽略
  - 对 `docs/` 前缀的文件路径自动忽略
...
**图表来源** 
- [scripts/harness.py:2218-2270](file://scripts/harness.py#L2218-L2270)
- [scripts/harness.py:1715-1735](file://scripts/harness.py#L1715-L1735)
...
- **新增** 证据清单相关问题：
  - 骨架文件缺失：检查 templates 目录权限和生成逻辑
  - 清单字段不完整：验证完成清单构建过程
  - 功能确认冲突：检查知识地图配置和功能匹配逻辑
- 调试建议：
...
**更新总结** 新增的证据清单与预生成骨架机制显著改善了用户体验，消除了首轮补证往返的痛点。功能确认支持与知识地图集成为复杂项目的证据管理提供了更精细的控制能力。这些改进使系统在保持向后兼容的同时，大幅提升了易用性和效率。
...
```json
{
  "required": ["test_result", "document_review"],
  "conditional": [],
  "required_receipts": [
    {"receipt": "write_set", "condition": "仅在任务产生实际写入时要求；无写入时不要求"},
    {"receipt": "read_set", "condition": "read_only 任务要求记录读取集"}
  ],
  "skeletons": [
    ".docs-harness/runs/dh-20240101T120000-abcdef/templates/evidence-test-result-skeleton.json",
    ".docs-harness/runs/dh-20240101T120000-abcdef/templates/evidence-document-review-skeleton.json"
  ]
}
```
