...
## 更新摘要
**所做更改**   
- 新增 .qoder/repowiki 外部知识库集成支持，实现外部只消费模式
- 添加知识卡片 frontmatter 解析功能，支持 name/scope/category 字段提取
- 实现基于任务文本和范围匹配的自动功能解析机制
- 更新知识状态检测逻辑，优先识别外部知识库源
- 增强知识引导流程，支持 external_consume_only 模式处理
...
**更新** 新增对外部知识库的支持，特别是 .qoder/repowiki 集成，提供外部只消费模式，无需本地知识库即可使用完整的知识管理能力。
...
- **新增** 外部知识库性能优化：知识卡片数量限制为 200 张，避免大规模扫描开销。
...
**更新** 新增的外部知识库支持进一步增强了系统的灵活性，允许项目直接使用现有的知识库资源，无需额外的初始化和维护成本。
