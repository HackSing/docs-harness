...
- 实现了增量write_scope扩展机制，支持每个任务包最多3次受控条件扩展
...
**更新** 本次更新重点增强了write_scope验证机制和引入了增量扩展能力，提供更严格的路径验证和更智能的范围扩展处理。
...
**更新** 新增了增量的write_scope扩展机制，支持在验证过程中智能扩展任务范围，提高自动化程度。
...
H-->>U : 处置结果(provide_evidence/refresh_evidence/retry_verification/incremental_admission/full_readmission)
```
...
- **新增** 增量扩展机制：当检测到越界写入时，系统会尝试在稳定合同条件下自动扩展 write_scope，最多支持3次增量扩展。
...
### 增量范围扩展机制（新增功能）
**新增** 系统现在支持在验证过程中智能扩展 write_scope，提高自动化程度：

- **扩展条件**：仅在 direct/planned 执行路线且非 git_sync 操作时启用
- **限制机制**：每个任务包最多支持3次增量扩展，超过限制需要重新准入
- **稳定性检查**：扩展前验证执行/授权/方案合同是否保持稳定
- **证据继承**：成功扩展后自动继承同轮已验证的证据收据
- **安全保证**：候选包必须是原范围与越界路径的超集，任何包覆写都失败关闭

```mermaid
flowchart TD
A["检测到越界写入"] --> B{"检查扩展条件"}
B --> C{"execution_route ∈ {direct, planned}?"}
C --> |否| D["拒绝扩展"]
C --> |是| E{"git_operation ≠ git_sync?"}
E --> |否| D
E --> |是| F["检查扩展次数 < 3"]
F --> |否| G["达到上限，需要重新准入"]
F --> |是| H["验证合同稳定性"]
...
- **新增** 增量准入：合同稳定且仅追加普通 Gate 时，原子增量 package revision，继承同轮已验证收据，降低重新准入成本。
...
- **新增** 范围验证错误：
  ...
  - `scope_extension_limit_exceeded`：已达3次增量扩围上限，需全量重新准入
...
## 结论
Docs Harness 的范围评估与授权系统以契约为中心，结合 Gate 与规则、严格的范围与授权收据、Git 预检/后检与证据收据，构建了高可靠、可审计、可扩展的任务执行与验收框架。**新增的增强验证机制和增量扩展能力**进一步提升了系统的智能化水平和用户体验。通过安全底线、增量准入与命令缓存等机制，在保证安全性的同时提升效率。遵循规则与合同，可实现稳定的授权策略与自定义扩展。
...
- **新增** 增量扩围：当检测到越界写入时，系统在稳定合同条件下自动扩展范围，最多3次，超过需重新准入。
