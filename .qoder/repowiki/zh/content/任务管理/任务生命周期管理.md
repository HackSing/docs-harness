...
**更新** 新增了增强的任务状态报告机制，包括证据检查清单（evidence_checklist）和待处理上下文收据（pending_context_receipts），以及改进的上下文加载失败处理，提供更详细的缺失上下文信息。
...
## 架构总览
任务生命周期由控制器统一编排，贯穿 run → context → verify → complete 的主路径，并在需要时进入计划与授权阶段。Git 操作通过预检与后检查确保边界安全。后台 Job 在父任务完成后按需消费冻结交付物。
...
- Gate 推断：基于关键词、路径分词与安全底线（security-sensitive/destructive-data/release-external）
...
**更新** 增加了新的使用场景，展示增强功能的使用方法。

- 只读查询：run → ready_direct → verify（无写操作）
...
**更新** 最新的增强功能进一步提升了系统的健壮性和用户体验：
- 增强的任务状态报告机制提供了更清晰的执行指导和恢复信息
- 改进的上下文加载失败处理确保了即使在宿主未遵循指引的情况下也能正确恢复
- 证据检查清单和待处理上下文收据的引入，实现了宿主与控制器之间的高效协作

建议在实际使用中遵循"最小必要变更"原则，充分利用幂等与缓存机制，提升整体效率与可靠性。
