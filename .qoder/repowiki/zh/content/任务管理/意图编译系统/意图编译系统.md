...
## 更新摘要
**所做更改**
- 更新了意图识别与候选生成部分，反映git_commit意图映射到git_metadata_write变更面的增强
- 增强了否定守卫系统的说明，包括对"提交证据/提交方案"等短语的误触发防护
- 更新了意图模式匹配机制，说明如何避免bare '提交'导致的误判
- 完善了测试用例引用，包含v1.7.5版本的git_commit意图边界测试
...
- 意图分类器
  ...
  - **新增**：git_commit意图现在映射到git_metadata_write变更面，具有更精确的权限控制
- 变更面与风险 Gate
  - 将意图映射为变更面（read_only/git_metadata_write/workspace_write/external_write）
  - 通过关键词与路径分词推断 Gate，安全底线 Gate 强制兜底并带否定守卫
  - **增强**：git_commit意图现在被正确识别为git_metadata_write而非workspace_write
- 范围与资源校验
...
- 变更面映射
  ...
  - **更新**：git_commit→git_metadata_write（新增映射）
  - 混合意图取最高变更面
- Gate 推断
...
**更新**：最新的git_commit意图映射和增强的否定守卫系统进一步提高了意图识别的准确性和安全性，有效防止了误触发和权限提升攻击。
...
- **新增**：本地提交意图
  - 任务："提交当前的用户改动，然后继续（只本地提交，不推送）"
  - 预期：task_intent=git_commit，mutation_profile=git_metadata_write，不包含git_fetch和external_write权限
- **增强**：否定守卫防护
  - 任务："提交证据清单"或"提交方案文档"
  - 预期：不会触发git_commit意图，避免bare '提交'导致的误判
- 证据与验收
  - 使用 docs-dash/evidence-receipt/v2；验证命令声明 produces；volatile 副产物白名单可控；未归因写入默认自动归因
...
- [tests/test_harness.py:7690-7760](file://tests/test_harness.py#L7690-L7760)
- [docs/contracts.md:165-221](file://docs/contracts.md#L165-L221)