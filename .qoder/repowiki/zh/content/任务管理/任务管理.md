...
COMP-->>CLI : admission_status + completion_manifest
CLI->>GIT : git_preflight_contract (fetch/sync)
CLI->>EV : 接收 evidence-receipt/v2 或声明草案
CLI->>BG : 若存在 background_deliverables，创建/消费 Job
CLI-->>U : 返回任务 ID、下一步、证据清单
U->>CLI : verify --task-id <id> [--evidence]
CLI->>EV : 校验证据/命令收据/工作区漂移
CLI->>GIT : git_postcheck (fetch/sync)
CLI-->>U : result=完成/需要补证/重试/增量准入/完整重新准入
```
...
Gates --> Admission["产出 admission_status 与 completion_manifest"]
Admission --> End(["结束"])
```
