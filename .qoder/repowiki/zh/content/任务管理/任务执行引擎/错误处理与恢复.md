...
alt 校验失败
Harness-->>CLI : 抛出 HarnessError(code, exit_code)
end
Harness->>Git : 预检(git_preflight_contract)
Git-->>Harness : 返回快照/阻断项
alt Git 预检失败
Harness-->>CLI : 返回 reason_code + blockers
end
...
## 结论
Docs Harness 通过统一的异常体系、严格的输入与范围校验、原子写与快照机制、事件日志与冻结快照、以及健壮的迁移与回滚流程，构建了高可靠性的错误处理与恢复框架。未来可在重试层引入指数退避与自适应退避策略，进一步提升对外部不稳定因素的韧性。
