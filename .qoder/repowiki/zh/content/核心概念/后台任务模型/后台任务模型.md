...
## 简介
本文件面向 Docs Harness 的后台任务模型，系统性阐述作业状态机、工作包管理机制、三种后台路线（background_direct、background_goal、background_goal_phased）的使用场景与限制、作业生命周期命令流程（prepare、progress、verify、retry）、失败处理与重试机制，以及调度配置与最佳实践。**最新更新**包括快速通道处理、改进的错误处理和性能跟踪指标。内容以源码与契约文档为依据，确保读者既能理解高层设计，也能定位到具体实现位置。
...
## 架构总览
后台任务由控制器统一编排，宿主通过 CLI 驱动状态迁移与工件操作。复杂路线需要宿主先创建 Goal/Plan，再由控制器校验并进入 running；简单路线可直接派发。**新增快速通道**允许低风险任务绕过完整审批流程，提升效率。所有控制面文件仅允许 CLI 写入，业务数据面受 allowed_write_scope 约束。
...
CLI->>Index : 记录摘要/事件
CLI-->>Host : 终态结果
Note over Host,CLI : 新增：快速通道支持低风险任务跳过完整流程
```
...
### 快速通道处理机制
**新增功能**：快速通道允许低风险任务绕过完整审批流程，显著提升处理效率。

- **触发条件**：
  - 路线为 direct
  - 未命中 high gate 与安全底线 Gate
  - write_scope 全部落在文档/规则/测试路径
  - 无 work_packages
- **处理方式**：
  - 声明制 fast_track: true
  - 证据集收敛为最小集（code_diff + test_run）
...
CheckFastTrack --> |是| ValidateConditions{"满足快速通道条件?"}
CheckFastTrack --> |否| NormalFlow["标准审批流程"]
ValidateConditions --> |是| FastTrackProcess["快速通道处理"]
ValidateConditions --> |否| NormalFlow
FastTrackProcess --> MinEvidence["最小证据集验证"]
...
**图表来源** 
- [docs/contracts.md:82-83](file://docs/contracts.md#L82-L83)
- [scripts/harness.py:4202-4223](file://scripts/harness.py#L4202-L4223)

**章节来源**
- [docs/contracts.md:82-83](file://docs/contracts.md#L82-L83)
- [tests/test_harness.py:691-803](file://tests/test_harness.py#L691-L803)
...
**章节来源**
- [scripts/harness.py:6807-6837](file://scripts/harness.py#L6807-L6837)
- [docs/contracts.md:82-83](file://docs/contracts.md#L82-L83)
...
- 建议：
  ...
  - **新增**：对于符合条件的低风险任务，考虑使用 fast_track 提升效率。
...
I["fast_track<br/>快速通道"] --> J["低风险任务优化"]
...
- 常见错误码与含义：
  ...
  - **新增**：fast_track_denied_reason：快速通道申请被拒绝的具体原因。
...
- 排查步骤：
  ...
  - **新增**：对于快速通道失败，检查 fast_track_denied_reason 获取具体拒绝原因。
...
## 结论
Docs Harness 的后台任务模型通过严格的状态机、确定的工件生成与校验、受控的写入范围与能力声明，实现了高可靠、可审计、可重试的后台治理流程。**最新增强**包括快速通道处理、改进的错误处理和性能跟踪指标，进一步提升了系统的效率和可维护性。正确理解 prepare、dispatch、progress、verify、retry 的语义与约束，以及新增的快速通道和优化特性，是稳定运行复杂后台任务的关键。
