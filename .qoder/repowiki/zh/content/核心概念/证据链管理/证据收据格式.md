...
- 生产者认证机制：可信生产者与报告型生产者的区别，capability 字段的含义与白名单约束
...
- 常量与工具：EVIDENCE_RECEIPT_SCHEMA、TRUSTED_EVIDENCE_PRODUCERS、sha256_text、utc_now、workspace_snapshot 等
...
章节来源
- [scripts/harness.py:5152-5201](file://scripts/harness.py#L5152-L5201)
- [scripts/harness.py:5118-5147](file://scripts/harness.py#L5118-L5147)
- [docs/contracts.md:165-216](file://docs/contracts.md#L165-L216)

### 生产者认证机制
- 可信生产者集合 TRUSTED_EVIDENCE_PRODUCERS 包含系统内置能力，如 git_postcheck、verification_command、auto_attribution、host_declaration 以及外部宿主能力 review_receipt、command_receipt、file_receipt 等
- 报告型生产者（reported）：由宿主自行生成的证据，trust_level 为 reported，不满足高风险证据类型要求
- 可信生产者（verified）：来自白名单的能力，trust_level 为 verified，可用于安全、发布、恢复等高风险证据类型
- capability 表示生产者具备的具体能力（如 review_receipt），与 adapter 共同构成唯一标识

章节来源
- [scripts/harness.py:240-249](file://scripts/harness.py#L240-L249)
- [scripts/harness.py:5182-5185](file://scripts/harness.py#L5182-L5185)
- [docs/contracts.md:165-216](file://docs/contracts.md#L165-L216)
...
章节来源
- [scripts/harness.py:5186-5192](file://scripts/harness.py#L5186-L5192)
- [docs/contracts.md:165-216](file://docs/contracts.md#L165-L216)
...
章节来源
- [scripts/harness.py:5050-5073](file://scripts/harness.py#L5050-L5073)
- [scripts/harness.py:5196-5197](file://scripts/harness.py#L5196-L5197)
...
章节来源
- [scripts/harness.py:5070-5073](file://scripts/harness.py#L5070-L5073)
- [scripts/harness.py:5193-5195](file://scripts/harness.py#L5193-L5195)
...
章节来源
- [scripts/harness.py:5128-5143](file://scripts/harness.py#L5128-L5143)
- [scripts/harness.py:5266-5283](file://scripts/harness.py#L5266-L5283)
...
- 控制器依赖：
  ...
  - 常量：EVIDENCE_RECEIPT_SCHEMA、TRUSTED_EVIDENCE_PRODUCERS
...
LoadEvidence --> TrustedProducers["TRUSTED_EVIDENCE_PRODUCERS"]
```
...
- untrusted_evidence_producer：生产者不在白名单，确认 adapter 与 capability 组合是否在 TRUSTED_EVIDENCE_PRODUCERS
...
章节来源
- [scripts/harness.py:5114-5201](file://scripts/harness.py#L5114-L5201)
- [docs/contracts.md:165-216](file://docs/contracts.md#L165-L216)

## 结论
证据收据 v2 通过严格的绑定字段、生产者白名单、时间戳与摘要校验，确保证据的可信性与可追溯性。控制器在装载与代铸过程中统一了宿主与系统的职责边界，配合工作区快照与漂移归因，形成闭环的验收体系。遵循本文规范可有效避免常见错误，提升证据质量与验收效率。
