...
章节来源
- [harness.py:385-392](file://scripts/harness.py#L385-L392)
- [harness.py:2300-2317](file://scripts/harness.py#L2300-L2317)
- [harness.py:2427-2452](file://scripts/harness.py#L2427-L2452)
- [harness.py:2859-2891](file://scripts/harness.py#L2859-L2891)

### 关键词推断回退逻辑
- 当未提供gate_assessment时，控制器回退到关键词推断：
  - 基于任务文本的宽泛术语匹配（GATE_DEFS.terms）。
  - 基于路径特征的启发式推断（文档、前端、测试、代码、架构契约等）。
  - 根据mutation_profile过滤不相关Gate（如read_only下排除code-edit/document-edit）。
- 该模式生成的gate_decision.mode为keyword_inferred，declared_gates为空，rationale为None，floor_added为空。
...
- [harness.py:2762-2776](file://scripts/harness.py#L2762-L2776)
- [harness.py:2300-2317](file://scripts/harness.py#L2300-L2317)
- [test_harness.py:5605-5623](file://tests/test_harness.py#L5605-L5623)
...
- 事实提交：
  - allowed_scope: ["src/**"]
  - gate_assessment: { "gates": ["code-edit"], "rationale": "单文件空指针修复，不涉及接口契约、安全与发布" }
- 预期结果：
...
章节来源
- [test_harness.py:5544-5564](file://tests/test_harness.py#L5544-L5564)

### 配置示例（底线强制并入）
- 目标：修复缓存逻辑并推送到远端。
- 事实提交：
  - allowed_scope: ["src/**"]
  - gate_assessment: { "gates": ["code-edit"], "rationale": "修复缓存逻辑，但任务包含远端推送" }
- 预期结果：
  - matched_gates: ["code-edit", "release-external"]
  - gate_decision.floor_added: ["release-external"]

章节来源
- [test_harness.py:5565-5584](file://tests/test_harness.py#L5565-L5584)

### 配置示例（路径触发底线）
- 目标：调整登录页展示逻辑。
- 事实提交：
  - write_scope: ["src/auth/**"]
  - gate_assessment: { "gates": ["code-edit"], "rationale": "调整登录页展示逻辑" }
- 预期结果：
  - matched_gates: ["code-edit", "security-sensitive"]
  - gate_decision.floor_added: ["security-sensitive"]

章节来源
- [test_harness.py:5585-5604](file://tests/test_harness.py#L5585-L5604)
...
章节来源
- [test_harness.py:5624-5634](file://tests/test_harness.py#L5624-L5634)
