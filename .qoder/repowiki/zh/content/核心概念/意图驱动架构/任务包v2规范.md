...
## 更新摘要
**所做更改**
- 在受控意图类型表格中添加`git_commit`意图，具有`git_metadata_write`变更面和`direct`执行路线
- 更新混合意图处理规则，说明`git_commit`意图的未来子句和完成体处理逻辑
- 修订Gate评估行为，明确显式facts只能升级Gate，不能降级
- 更新标准JSON配置示例，包含`git_commit`意图的标准定义格式
...
- 受控意图类型：query、audit、git_inspect、git_fetch、git_sync、git_commit、modify、external_write
...
| git_commit | git_metadata_write | direct | 本地提交层操作，写 .git 对象/索引/分支引用，不改工作区、不触远端 |
...
**Updated** 添加 `git_commit` 意图，支持本地提交操作，具有 `git_metadata_write` 变更面和 `direct` 执行路线，允许本地提交但不触发远端交付层。
...
**Updated** `git_commit` 意图在未来子句（如"后续再提交改动"）时进入 deferred_intents，在完成体（如"已经提交了改动"）时仅作为审查上下文。
...
- allowed_actions 由意图与 mutation_profile 推导，例如：
  - read_only 通常允许 ["read"]
  - git_metadata_write 允许 ["git_fetch", "git_commit"]
  - workspace_write 允许 ["git_inspect", "git_sync"] 等
  - external_write 允许 ["external_write"]
- 测试用例覆盖了不同意图下的 allowed_actions 推导结果

**Updated** `git_metadata_write` 变更面现在同时允许 `git_fetch` 和 `git_commit` 操作，但 `git_commit` 不附带 `git_fetch` 授权，也不触发远端交付层。
...
**Updated** Gate 评估行为修订：显式 facts 只能升级 Gate，不能把 `audit+fix`、`if-needed-fix` 或 `fetch+sync` 降级。任务意图的英文短词仍按单词边界识别，Gate 路径推断按完整路径段匹配。
...
- git_commit：git_metadata_write 变更面，direct 路线，allowed_actions 包含 git_commit，不附带 git_fetch 授权
...
**Updated** 新增 `git_commit` 意图的标准定义格式，适用于本地提交场景，具有 `git_metadata_write` 变更面和 `direct` 执行路线。
