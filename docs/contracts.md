# Docs Harness 2.0.0 产品合同

本文件只描述 2.0.0 当前能力。1.x 的 Run、Gate、Evidence、Verify、Readmission、规则和后台 Job 已从可运行合同移除；旧项目统一按 [2.0.0 单向迁移指南](migrations/v2.0.0.md) 升级。
...
成功结果写入 `docs-harness/plan/v2` 与选择指纹。执行阶段只消费 `execution_projection` 中的目标、范围、步骤、约束和验收，不继承模板选择过程、候选讨论或控制器状态。
...
`acceptance record` 只登记已经发生的验收，不自动执行测试，也不决定应该跑全量还是聚焦验证。
...
L1 不能声明行为正确；Behavior Acceptance 只能记录 L2–L4；L5 必须由用户确认。独立 CLI 不接受 `user_acceptance + passed`，只能记录 `user_pending`。

通过必须提供实际方法和项目内已存在的常规证据文件。失败必须提供唯一原因和下一步。`user_pending` 必须包含已自动验证内容、待用户检查项、最短步骤和 `environment_ready=true`。
...
`run|context|progress|verify|task|background|authorization`、`--legacy-opt-in` 和知识维护 Job 已从 2.0.0 CLI 与控制器删除。2.0.0 不读取、创建、继续或验证 1.x 任务状态；旧项目只通过 `project upgrade` 执行单向迁移。
