...
每次结论标注最高真实层级：L1 源码一致性、L2 聚焦行为、L3 本地运行态、L4 包/安装、L5 用户验收。Contract Check 不能代替 Behavior Acceptance。
...
- Acceptance 的 L1–L5 边界、真实证据文件和 L5 用户待确认；
...
2.0.0 不再运行 1.x 状态机全量回归，因为该能力已经从当前产品和包中删除。迁移回归只验证现存升级合同：fresh init、旧项目 preview/apply/diff/check、所有权清理、项目内容保留、冲突零写入、旧命令缺席和重复升级幂等。
