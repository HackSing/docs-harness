...
- 补齐 2.21.0 迁移用户全局 CLAUDE.md 时漏掉的两句，补完后全局文件可以退役：
  - 「收尾」开头加「结论先行，不把推理过程当答案」，待决事项放在结论之后。
  - 「完整不等于扩大范围」的范围扩张清单加「优化没人抱怨的性能」。
- 不改命令契约、Plan 模板字段与资产流程。
- **所有下游下次 `project upgrade --apply` 会在 `AGENTS.md`/`CLAUDE.md` 上产生一次受管区块 diff。**

## 2.21.0 - 2026-09-24

- 受管入口新增「协作准则」节，放在「工作流规则」之前（动机：所有项目都已接入 Harness，用户全局 CLAUDE.md 里的通用协作习惯迁入受管入口，这样也能随 AGENTS.md 覆盖 Codex 等非 Claude 代理）：
  - 按意图交付：先用一句话说出用户的目的，按目的交付；只影响交付完整度的歧义按更完整的理解做并写明假设，会改变目标、方案或验收的歧义仍按工作流规则第 1 条先确认（与原「验收先行」确认点划清边界，不冲突）；附常见请求与交付形态的映射。
  - 前提有误先纠正：第一句先说清哪部分对、哪部分不对；被指出做错时先把受影响的产出改回来再继续。
  - 完整不等于扩大范围：交付物缺的部分自己补上，重构、改配置、删文件、提交代码等范围扩张先问用户。
  - 可开工自检：接收方能否不追问就开工。
- 「验收以真实功能为中心」补非运行态交付物的验证方式：方案做路径推演，核对换一条路径重算，文档以零基础读者身份通读。
- 「收尾」待决事项含所做假设与偏离要求之处；多个问题按严重性排序；失败、跳过、未完成的部分单独写明。
- `evals/evals.json` 新增 `intent-and-premise-first` 行为用例。
- 不改命令契约、Plan 模板字段与资产流程。
- **所有下游下次 `project upgrade --apply` 会在 `AGENTS.md`/`CLAUDE.md` 上产生一次受管区块 diff。**

## 2.20.0 - 2026-09-23

- 受管入口工作流规则第 2 条「根因优先」补两句（动机：规则只要求"找根因、列影响面"，没区分阅读面与修改面，"最小方案"的压力会被模型顺带理解为"读得少"，漏读消费者；方案成型后也没有针对"是否治本、是否最小"的检查点，笼统自问会被自我确认混过去）：
  - 定位读够、修改最少：上溯到状态所有者、下查到全部消费者，调用方用符号检索确认、不凭 CODEMAP 推断；"最小改动"只约束修改面，不得借此缩小阅读面。
  - 方案交给用户前（bug 修复、需用户确认的方案、Plan）按四问逐项自检：所有者层修复、影响面逐条消除、逐处删改验证无多余改动、无证据的兜底/重试/兼容分支；任一不过先改方案再输出，输出附一行自检结论，被修正时写明改了什么。简单任务不触发。
- `evals/evals.json` 新增 `fix-plan-self-check` 行为用例。
- 不改 bugfix Plan 模板字段（已有 `fix_boundary`、`affected_modules`）与命令契约。CODEMAP 是否为隐式耦合加字段登记为 TODO，待下游证据。
- **所有下游下次 `project upgrade --apply` 会在 `AGENTS.md`/`CLAUDE.md` 上产生一次受管区块 diff。**

## 2.19.0 - 2026-09-23

- 受管入口模板按 Opus 5.5 行为补四处（动机：官方指南《Getting the most out of Opus 5.5》指出该模型长任务中倾向停下汇报、会按写明的停止条件执行；原模板只写了何时停，没写何时继续）：
  - 工作流规则总述：需要停下等用户的只有本文确认点、用户另有要求、原生授权提示三种；其余步骤直接继续，进度说明与下一步动作同一条消息给出，不以阶段汇报、"是否继续"征询或不阻塞的选项清单收尾。
  - 第 4 条分批交付："锁定"注明是验证门、不是停下汇报的点；批次划分写入进度清单。
  - 第 5 条并行优先：主 agent 核对证据支持结论后才采纳子智能体结论，不直接转述。
  - 收尾：先列需要用户决定或确认的事项（没有写"无"），再报改动、命令、验收层、未覆盖项与风险。
- 新增本地约定目录 `.docs-harness/tasks/`：预计跨多批次或可能经历上下文压缩的长任务，进度清单写入 `.docs-harness/tasks/<任务名>.md`，查进度以文件为准；有 Plan 时清单开头写 Plan 路径、只记进度不复述内容，进度不写回冻结合同。与 `inputs/` 共用一份判定：`task_inputs_changes`/`apply_task_inputs_dir` 泛化为 `local_only_dir_changes`/`apply_local_only_dirs`，遍历新常量 `LOCAL_ONLY_DIRS`（`TASK_INPUTS_GITIGNORE_CONTENT` 更名 `LOCAL_ONLY_GITIGNORE_CONTENT`）；嵌套 `.gitignore` 写法仍只有 harness 与 `usage_log` 两处，未到第 3 次抽象阈值。init 与 upgrade 均确保其存在，已存在的 `.gitignore` 不覆盖，不在 `LEGACY_RUNTIME_NAMES` 内、升级不清理。
- `docs/contracts.md` §1 同步停顿点合同，§7 安装面补 `tasks/` 条目；`evals/evals.json` 新增 `long-run-keeps-going`、`subagent-evidence-checked` 两条行为用例。
- **所有下游下次 `project upgrade --apply` 会在 `AGENTS.md`/`CLAUDE.md` 上产生一次受管区块 diff，并新建 `.docs-harness/tasks/.gitignore`。**

## 2.18.0 - 2026-09-15

- 受管入口模板补并行执行指引（`_managed_content()` 首条与 `_GENERIC_STANDARDS` 工作流规则、结构护栏）。动机：TODO 登记的实测缺口——模型无明确提示时不主动开子智能体，独立检索、多文件实现一律串行；且入口首条"默认由 agent 直接完成"本意是不经 Harness 流程，字面读却像要求主 agent 亲自串行干完，进一步抑制委派；工作流规则第 4 条"改完 → 验证 → 锁定 → 下一批"对所有批次一刀切串行，没有区分有依赖与独立批次。五处改动：首条改为"默认不经 Harness 流程直接执行"并写明"直接"不指亲自串行、指向第 5 条；第 3 条回归必跑补"消费者跨两个以上模块时分头并行确认"；第 4 条分批交付重写为每批标注依赖（`B2 ← B1` 或 `独立`）与文件范围，有依赖串行、独立且文件范围不相交的并行、主 agent 统一集成验证再锁定；新增第 5 条并行优先，按分量三级判定（单点直接做 / 轻量独立子任务用同消息并行工具调用不开子智能体 / 分支够重才并行开子智能体），写死任务书四要素（目标、验收、路径范围、文件白名单）、汇报只回结论与证据路径、四类保持串行的边界（同一文件、受管公共文件、依赖步骤、批次验证门）与"并行不豁免集成验证"；结构护栏第 5 条搜索面收敛补"子智能体检索同受路径范围约束"。"够重"按用户决定保留判断型措辞，不给可数阈值，待下游实测后再决定是否量化。受管块 5546 → 6100 字（+554 字，估算 +324 token，约 11%），判断型规则其余一字不动。
- `docs/contracts.md` §1 默认合同同步消歧："普通任务不经 Harness 流程直接执行"并写明不约束主 agent 是否拆分或委派。
- 移除 `.qoder/repowiki` 外部知识源的全部残留（常量 `REPOWIKI_RELATIVE`、`_managed_content()` 的入口条件分支、`knowledge query` 候选来源、`project_knowledge_summary()` 的 `source="repowiki"` 分支）。动机：2026-09-15 对 zbuddy-desktop/zbuddy-mobile 的 usage 与会话记录审计——入口在目标存在 `.qoder/repowiki` 时把 agent 先导向 repowiki 而非 `docs/knowledge/`，但 desktop 的 repowiki（193 个 md）自 08-14 起未再生成，且全部 CCD 会话记录中没有任何一次读取 `.qoder/repowiki/zh/content`；该行既是死路径，又把 agent 推离活的 Knowledge 资产。三处分支均无测试覆盖，属 1.x 时代遗留（见 1.x CHANGELOG 的 repowiki 条目）。行为变化：入口 knowledge 行不再随目标目录变化，统一为"先查当前源码与符号；仍缺关键事实再显式运行 knowledge query"；`knowledge query` 不再扫描 `.qoder/repowiki/**/*.md`（desktop 样例查询 omitted 由 304 降到 248，56 条候选来自 repowiki；top-5 命中与输出体量不变）；`project check/init/upgrade` payload 的 `knowledge.source` 不再出现 `repowiki`，无受管资产且有 `docs/` 时回 `project_docs`。`_managed_content()` 不再需要 `target` 参数。
- usage 事件补错误码：`usage_invoke_event()` 在 `result=error`（`HarnessError` 路径）时新增可选键 `error_code`，取 `payload["code"]` 标识符枚举，不记错误 message；schema 版本不变，dry-run 校验不通过返回 `dry_run_invalid` 结果，不写该键。动机：2026-09-15 复核 zbuddy-desktop，一次完整任务里 11 次首次失败只记了 `result=error`，该任务不在 CCD 会话内，原始报错已不可找回，摩擦出在哪条命令、哪种输入无从判断。
- `usage report` 输出契约修订：`commands` 每项新增 `errors`（只数 `result=error`，不从退出码推断）与 `error_codes` 分布；`checks` 每项由跨调用累计的 `failures`/`warnings` 改为最近一次调用的 `last_failures`/`last_warnings`。动机：pre-commit 每次提交都跑检查，未处置的同一条 WARN 被反复计入，desktop 47 次检查累计 518 条实为 12 条方案告警重复计数；退出码分桶也让 `acceptance record` 退 3（记录已存入、整体待定）被误读成失败。**`checks.*.failures`/`checks.*.warnings` 两个键被移除，依赖旧键的跨仓库汇总需改读新键。** 合同 §9.1/§9.4 与 Knowledge `usage-observability`（修订 3）同步。
- `plan check` 符号全命中 WARN 新增第三出口：横幅写 `状态：有效-部分交付（YYYY-MM-DD 核对）` 后，核对日起 `PLAN_PARTIAL_DELIVERY_RECHECK_DAYS`（30）天内不报该 WARN，过期、日期非法或未来日期照常报；判定为 `partial_delivery_checked_recently()`，时效内的方案不参与符号扫描。WARN 文案同时列出已交付、不再推进与部分交付三个出口，`plan check --help`、合同 §3.3、SKILL 同步；受管入口判定纪律补"不得为消除 WARN 而 settle"。动机：desktop 当前 18 条 WARN 中 12 条是这一条，09-14 审计记录其中多数属于部分交付、按规定应保持有效，而 WARN 只给两个结算出口，每次提交原样重复，淹没真告警。
- `plan settle` 接受无伴随 JSON 的手写方案：识别条件是无同名 JSON、前 3 行无 Harness 文档标记且有状态横幅（`plan_settle_paths()` 的 JSON 位返回 `None`，分派到 `settle_handwritten_plan()`）。`implemented` 改写横幅，条目在受管方案区块内时同步改写条目状态；`deprecated` 复用 `settle_deprecated_plan()` 改写横幅、移入 `archive/` 并改写链接。手写方案没有冻结治理合同，不做治理终验，传 `--governance-input` 以 `invalid_plan_transition` 拒绝；受管区块外的 INDEX 条目属项目正文不改写，结果带 `handwritten=true` 与 `warnings` 提示手工同步；带文档标记却缺 JSON 的方案仍以 `invalid_plan_ref` 拒绝。`replace_plan_status_banner()` 统一保留横幅首个「｜」之后的附注（托管方案横幅不含该字符，行为不变）。受管入口 Plan 条补"手写方案同样直接 settle，不得手工补造冻结 JSON"，`--help`、合同 §3.3、SKILL、README 同步。动机：desktop 21 个手写方案无伴随 JSON，09-14 审计批量结算同一秒 7 次被 `invalid_plan_ref` 拒绝，最后靠手工构造 JSON 补登。不新增 `plan register`：为从未冻结过的方案补造冻结合同与指纹，等于伪造执行合同。
- 修复 `test_project_check_flags_non_executable_githook_index_mode` 的平台相关失败（仅测试）：用例未钉死基线提交的钩子模式，`core.filemode=true` 的 macOS 按磁盘可执行位入库 100755，chmod 往返后二次提交报 nothing to commit；0578fef 为此删掉二次提交，又让 `core.filemode=false` 的 Windows（基线 100644）在加 +x 后索引与 HEAD 不一致，`project check` 回待提交退 3。现在用例内设 `core.filemode false` 钉死基线为 100644，按真实修复路径加 +x 后提交。Windows 本机通过，macOS 未实跑。
- `tests/test_cli_surface.py` 的 `harness.py` 行数与字节硬上限第 8 次上调（行数 4850→4900，实际 4851；字节 210000→220000，实际 213765）：本版控制器只增 usage 错误码接线、部分交付判定与手写方案结算分支，聚合逻辑在 `usage_report.py`；存量体量债仍由 TODO 登记的专门整理任务处置。
- `evals/evals.json` 新增两条行为用例：`parallel-independent-branches`（批次依赖标注、独立分支并行、任务书带路径范围、汇报只回结论、主 agent 集成验证、受管公共文件串行）与 `parallel-light-tasks-no-spawn`（同消息并行工具调用、零子智能体、单点任务直接做）。此前 19 条用例无一覆盖委派或并行。
- 未做：Full Plan 不新增调度字段（`execution_graph`）。按"抽象同样要证据"规则，先以提示词规则跑一个版本，若下游审计仍见串行再升格为合同字段。dsh-plugin 的 vendor 引擎仍停在 2.12.3，需单独走插件发版（`seed-vendor` + `extract-block`）才能把本条规则带给插件用户。
- **所有下游下次 `project upgrade --apply` 都会在 `AGENTS.md`/`CLAUDE.md` 上产生一次受管区块 diff**（受管入口模板契约变化，非行为变更）。

## 2.17.0 - 2026-09-13

- 受管入口模板（AGENTS.md/CLAUDE.md 受管区块、`_managed_content()` 与 `_GENERIC_STANDARDS`）第一次瘦身：受管块从 2.8.0 的 5342 字一路涨到 2.16.2 的 6847 字，八个 minor 版本只增不减，每次新增能力都往入口里追加一句流程细节，而弱模型每个任务都要读完整块。本版只下沉"工具已经兜住"的流程细节，判断型规则（工作流规则、编码质量规范、结构护栏、防御代码准入、提示词与代码流程同步、测试与验收范围、收尾七节）一条不动。改后受管块 5626 字（-1221，-17.8%）。
- 下沉依据逐条对应到既有兜底：plans 文档卫生的四条细则（状态横幅取值、索引带 2-4 个符号、废弃归档 sweep 死链）由 `command_plan_check()` 把关（docstring 即"横幅、索引闭环、死链、取值、符号存活性与时效"），pre-commit 的 `assets-check --fast` 与 CI 的 `--strict` 已包含它，入口只需留"由 plan check 把关 + 判定纪律"；`--selection` 可传 `selection_ref`（sha256 指纹）与 `create --dry-run` 预检已写在 `plan create --help`／`acceptance create --help` 的正文末两行，入口改为一句"形状、必填字段与 --dry-run 预检见 `<cmd> --help`"；证据落在 git 忽略路径由 `assert_evidence_usable()`（经 `git_ignored_refs()`）在登记入口直接拒绝并回 `acceptance_evidence_ignored`，入口只留"必须位于随仓库提交的路径"这条正向要求；`acceptance_required=true|false` 与 `knowledge_impact=updated|unchanged` 的取值由 `scripts/plan_governance.py` 的 Full Plan 抽取与 settle 校验硬拦，入口改为"字段在 settle 时校验"。
- `## 文档可发现性规范（plans 文档）` 一节与其后的 Plan/Knowledge/Acceptance/assets-check 散文段落合并重写为 `## 方案、知识与验收资产` 六条 bullet：同一主题此前分成"编号细则 + 四段散文"两处，命令名、时机与判定纪律重复出现，合并后每条资产一行、命令顺序与判定纪律各只出现一次。
- 同批追加的四处去重与删历史句（版本号不再升，仍属 2.17.0 一次契约变更）：Plan/Acceptance 的命令序列此前在第一节与 `## 方案、知识与验收资产` 各讲一遍，本次全部合并到资产节（Plan bullet 收 `Knowledge → Acceptance → Plan` 结算顺序，Acceptance bullet 收"Plan 后创建"与证据路径要求），第一节只留一句"何时才走资产流程"并指向资产节；删掉 `pre-2.0 项目只通过 project upgrade 单向迁移` 一句——迁移全由 `project upgrade` 自己完成，agent 侧没有任何对应动作，这是 1.x→2.0 的历史交代而非当前指令；删掉受管块正文首行 `Docs Harness 当前版本：<version>`（紧邻的标题 `## Docs Harness <version>：默认直跑，能力按需` 已含同一版本号，该正文行只剩下在 `tests/test_project_upgrade.py` 的 1.8.2 legacy 夹具里作为旧格式标记，不受本次影响）；删掉 ADR 括号 `（定稿不可改，复杂决策可选只读子智能体复审）` 与结构护栏第 3 条里 Structure 增量检查的实现描述 `（对比 HEAD，只对本次改动归责）`——两处都是工具行为的复述，`adr create` 的定稿约束与 `structure check` 的归责口径都由命令自身实现，入口只需留触发条件。
- **所有下游下次 `project upgrade --apply` 都会在 `AGENTS.md`/`CLAUDE.md` 上产生一次受管区块 diff**（受管入口模板契约变化，非行为变更）。无测试需要改写：现有用例（`tests/test_cli_surface.py` 的 `test_prompt_surfaces_carry_input_schemas_and_managed_blocks_synced`）只断言两个入口文件与 `managed_agent_block(ROOT)`/`claude_block(ROOT)` 逐字一致，加上三处跨版本稳定的锚点（`<cmd> --help` 指向、体量预警两句），本次改写都不触碰这些锚点。
...
- 修复 `package.json` `files` 的死条目 `tests/test_v2_direct.py`（文件已不存在，43→42 项）。动机：2.16.0 新增的 `test_package_files_cover_every_managed_module` 只查漏项（真源→清单），不查死项（清单→磁盘），而 `npm pack` 对不存在的条目静默跳过，清单因此可以无限期腐烂。新增反方向守卫 `test_package_files_entries_all_exist`：逐项断言存在，以 `/` 结尾按目录判。两条守卫互补、不合并。
- 新增一次性输入 JSON 的约定位置 `.docs-harness/inputs/`（常量 `TASK_INPUTS_RELATIVE`）：`project init` 与 `upgrade` 都确保目录存在并落一个内容为 `*` 的嵌套 `.gitignore`，已存在则一律不覆盖。动机：`plan create --content`、`acceptance record --input` 等都要求输入文件位于项目内，但此前没有约定位置，而 1.x 运行态目录 `.docs-harness/task-inputs/` 在 `LEGACY_RUNTIME_NAMES` 内会被 `project upgrade` 清理——2.16.0 任务为此三次重建备份。新目录不在该元组内、升级不清理；`task-inputs` 的 legacy 清理行为不变，两者不做迁移。受管入口模板同步追加一句写明该位置（提示词与代码流程同步第 2、5 条），因此所有下游下次 `project upgrade --apply` 都会在 `AGENTS.md`/`CLAUDE.md` 上产生一次受管区块 diff。新增用例 4 条（`tests/test_project_install.py`）。
- 新增 usage 观测首次开启的一次性告知：v12 及更早 → v13 的那一次 `project upgrade`，预览与 `--apply` 两条 payload 都带 `notices`，说明日志位置、不入库不外发、以及关闭方法。动机：2.16.0 让 `usage_log` 在 config v13 默认开启，下游升级后立即开始记录，但升级输出没有任何提示，知情渠道只有 CHANGELOG/contracts §9/ADR。条件是升级前 config 缺 `usage_log` 键且新值为开，因此 fresh `init` 不提示、已是 v13 的项目再升级也不重复提示。**已升到 2.16.0 的下游收不到这条提示**——它们的 config 已是 v13，这个一次性窗口已经过去，对这批项目的告知渠道仍然只有本 CHANGELOG 与 contracts §9；本版不做补发机制。新增用例 3 条（`tests/test_project_upgrade.py` 两条端到端 + 一条真值表）。
- `tests/test_cli_surface.py` 的 `harness.py` 行数硬上限第 7 次上调（4700→4850，实际 4800 行）。本次改动使其越线。两道体量闸的分工：本地由 Structure 增量 WARN 触发结构评估；CI 由本上限硬拦——Structure 增量检查对比 HEAD，提交后增量恒为空，CI 的 `assets-check --strict` 永远看不到 `harness.py` 的体量 WARN，因此这条硬上限是 CI 中唯一拦得住 `harness.py` 无限增长的闸。上限的去留与 4800 行存量债由 `TODO.md` 登记的专门整理任务决定，不在功能任务里顺手重构。
...
- 新增 Structure 结构护栏检查（`scripts/structure_check.py`）作为 `assets-check` 第六个 checker，全部 WARN 级：增量体量检查只对本次改动（工作区+暂存+未跟踪 vs HEAD）归责——新增文件超 500 行红线、既有文件本次净增突破红线、已超标文件单次净增 ≥50 行、Python 新增函数超 60 行红线或既有超标函数单次增长 ≥10 行（`ast` 测量，零依赖；其他语言只做文件级）。动机：存量阈值检查对遗留大文件恒报会造成 WARN 疲劳，增量归责让每条警告都指向本次任务；WARN 不 FAIL，拆分与豁免的处方权留给人。非 git 目标 checked=0 跳过，与 ScriptHygiene 同口径。
- 新增 `docs/CODEMAP.md` 代码能力索引：条目格式「模块路径 — 职责 — 公开接口」；Structure 检查校验登记路径存在、公开接口符号存活，并对本次新增且未登记的代码文件（测试文件豁免）出 WARN——复用失败的根因是检索失败，索引让"先复用后新写"有稳定入口。init/upgrade 缺失即建 CODEMAP 脚手架，已存在零覆盖；无 CODEMAP 的项目整体跳过相关检查，不产噪音。
- 新增顶级命令 `structure`：`structure check` 单独运行增量检查（分批交付的批次验证点用）；`structure report` 输出存量结构债报告（超红线文件/函数、CODEMAP 覆盖缺口），供定期整理任务消费——增量检查不可见的既有债由报告按需暴露。
- Full 方案模板新增必填字段 `module_interfaces`（模块划分与接口骨架）：复杂任务动手前冻结模块清单、对外接口符号与复用路径，实施骨架先行；brief 级不变，简单任务零负担。受管入口同步新增「结构护栏」规范与「抽象同样要证据」条款（不为尚未出现的复用场景预留抽象层）。
- 受管入口运行模式清单合入下游 dispatch/ZBuddy 以 2.10.3 号验证的纪律行：改动涉及用户可见行为、对外接口或命令契约、版本发布时同步更新 CHANGELOG，任务产生待跟进事项时登记 TODO；2.10.3 版本号就此消化，两项目升级到本版不丢补丁。
- 安装配置升级为 `project-config/v12`（新增 structure_check 模块指纹），v1-v11 单向平滑升级。升级预检依赖各项目 config 自带模板指纹（经核实全部下游项目指纹健康），不新增 legacy 模板指纹条目。
- 版本说明：本条目功能曾在本地分支以 2.10.0 号交付，与远端已发布的 2.10.0（证据准入加固）撞号；收敛时在 2.10.2 基础上重建并重标为 2.11.0，原始提交保留于 backup/pre-converge 分支。
