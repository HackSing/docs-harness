...
- 宿主兼容：dsh-buddy 的 Anchored Standard 预设在修复前（≤ 2026-08-18 构建）用 scope 静态 `complete: true` 压制全部插件 prompt section，本插件治理段整个会话不可达；修复版（相位化密封）起治理段在锚定建立后正常注入，harness_plan_* 经 `extraIndex` 可搜索解锁。
