...
class StructureGuardrailTest(HarnessTestBase):
    ...
    def test_structure_check_warns_on_new_oversized_file_and_function(self) -> None:
        ...
        self.write_lines("src/big.py", [f"x{i} = {i}" for i in range(501)])
    ...
    def test_structure_check_increment_growth_rules(self) -> None:
        self.structure_git("init")
        self.write_lines("mod.py", [f"a{i} = {i}" for i in range(480)])
        self.write_lines("fat.py", [f"b{i} = {i}" for i in range(600)])
        self.write_lines(
            "funcs.py",
            ["def stable():"] + ["    pass"] * 64 + ["def grower():"] + ["    pass"] * 54,
        )
        self.structure_commit_all()
        self.write_lines("mod.py", [f"a{i} = {i}" for i in range(520)])
        self.write_lines("fat.py", [f"b{i} = {i}" for i in range(660)])
        self.write_lines(
            "funcs.py",
    ...
    def test_structure_check_small_growth_stays_silent(self) -> None:
        ...
        self.structure_commit_all()
        self.write_lines("fat.py", [f"b{i} = {i}" for i in range(610)])
    ...
    def test_structure_report_lists_stock_debt(self) -> None:
        self.structure_git("init")
        self.write_lines("legacy.py", ["def whale():"] + ["    pass"] * 80 + [f"c{i} = {i}" for i in range(520)])
        self.structure_commit_all()
    ...
    def test_assets_check_carries_structure_warnings(self) -> None:
        ...
        self.structure_git("init")
        self.structure_commit_all()
        self.write_lines("src/huge.py", [f"h{i} = {i}" for i in range(501)])
